namespace EvolAlgoLevelGenerator {
	public class EvolutionaryAlgo<TIndividuum> {
		...
		public EvolutionaryAlgo(EvolAlgoParameters<TIndividuum> parameters, bool quiet = true) {
		...
		public IEnumerable<TIndividuum> Run() {
			...
			int generations = 0;
			IEnumerable<Scored<TIndividuum>> population
				= _parameters.Initial
			...
			while(! _parameters.EndingPredicate(state)) {
				...
				var offsprings =
					Enumerable.Range(0, (int)Math.Ceiling(iterations))
					.Select(i => _parameters.ParentSelector.GetParents(
							population,
							_parameters.Breeding.ParentCount
						))
					.Select(list => list.Select(ind => _parameters.MutateFunc(ind.Value)).ToList())
					.Select(p => _parameters.Breeding.Breed(p))
					.SelectMany(x => x)
		...
		private double GetTime() => DateTime.Now.Ticks;
	}
}