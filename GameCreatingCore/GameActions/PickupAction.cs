...
namespace GameCreatingCore.GameActions
{
    public class PickupAction : IGameAction {
		...
		public LevelStateTimed CharacterActionPhase(LevelStateTimed input) {
			var dist = Vector2.SqrMagnitude(input.playerState.Position - _level.SkillsToPickup[_pickupableIndex].position);
			...
			if(newTime > _pickUpTime) {
				var act = _level.SkillsToPickup[_pickupableIndex].action;
