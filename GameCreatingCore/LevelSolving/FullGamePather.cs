...
namespace GameCreatingCore.LevelSolving
{
    public class FullGamePather : IGamePathSolver
    {
        ...
        public FullGamePather(int innerViewconeRayCount, float viewconesGraphDistances,
            ...
            int maxIterations,
            StaticNavGraph? staticGraph = null, bool inflateObstacles = true, float initialWait = 0)
        {
            ...
            bool path)
        {
            ...
            var initLevel = GameSimulator.GetInitialLevelState(levelRepresentation, staticGameRepresentation, staticGraph);
            ...
            Func<ScoredActionedNode, ScoreHolder?> scoreHolderFunction)
        {
            ...
            initLevel = DoInitialWait(initLevel, simulator, viewGraph, out var lost);
            if (lost)
            {
                return (null, new List<StateNode>());
            }
        ...
        LevelState DoInitialWait(LevelState state, GameSimulator simulator, IViewconesBearer viewGraph, out bool lost)
        {
            float usedTime = 0;
            lost = false;
            while (!FloatEquality.MoreOrEqual(usedTime, initialWait))
            {
